...
$sql_sem="SELECT count(*) as cant FROM cdr WHERE date(start_stamp)>=\"$sem\" and length(destination_number)>4 and length(caller_id_number)=4 and date(start_stamp)<=date('now')";
...
$sql="SELECT count(*) as cant from cdr where length(destination_number)>4 and length(caller_id_number)=4 and date(start_stamp)=\"$ante_ayer\";";
$ret=$db->query($sql);
$row=$ret->fetchArray(SQLITE3_ASSOC);
$llamadas_ante_ayer=$row["cant"];

$sql="SELECT count(*) as cant from cdr where length(destination_number)>4 and length(caller_id_number)=4 and date(start_stamp)=\"$ayer\";";
$ret=$db->query($sql);
$row=$ret->fetchArray(SQLITE3_ASSOC);
$llamadas_ayer=$row["cant"];

$sql="SELECT count(*) as cant from cdr where length(destination_number)>4 and length(caller_id_number)=4 and date('now')=date(start_stamp);";
$ret=$db->query($sql);
$row=$ret->fetchArray(SQLITE3_ASSOC);
